...
public abstract class TownCell {
	...
	public void census(int nCensus[]) {
		...
		//Sets bounds to iterate through
		int ceil = Math.max(0,  row - 1);
		int floor = Math.min(plain.grid.length - 1, row + 1);
		int left = Math.max(0, col - 1);
		int right = Math.min(plain.grid.length - 1, col + 1);
		
		//Iteration through the grid using who() to find cell type
