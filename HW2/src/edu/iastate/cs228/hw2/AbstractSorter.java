...
public abstract class AbstractSorter
{
	 ...
	 *        
	 */
	protected void setComparator(int order) throws IllegalArgumentException
	{
		if (order < 0 || order > 1)
		{
		...
		//Sets order based on 'order' throws IAE if not 0 || 1
		else {
			if (order == 0)
				Point.setXorY(true);
	 ...
	 * @param pts
	 */
	private void getPoints(Point[] pts)
	{
		points = pts;
	}
	...
	protected void swap(int i, int j)
	{
		...
		//creates a temp variable to hold value during switch
		temp = points[i];
		points[i] = points[j];
		points[j] = temp;
	}	
